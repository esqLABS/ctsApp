...
test_that("Test undisplay works", {
  ...
  # shiny >= 1.8.0 wraps label in <span class="action-label">
  expect_true(
    grepl('id="go_filter"', as.character(b)) &&
    grepl('class="btn btn-default action-button"', as.character(b))
  )
  b_undisplay <- undisplay(b)
  expect_s3_class(b, "shiny.tag")
  expect_true(
    grepl('style="display: none;"', as.character(b_undisplay)) &&
    grepl('id="go_filter"', as.character(b_undisplay))
  )
